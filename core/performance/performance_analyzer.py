...
class EnhancedAMOTraceAnalyzer:
    ...
    def _attach_trace_handler(self, trace: Any) -> None:
        if self._trace_handler_attached or trace is None:
            logger.debug("Skipping handler attachment: already_attached=%s, trace_is_none=%s",
                        self._trace_handler_attached, trace is None)
            return
        ...
        try:
            ...
            logger.debug("Event handler attached successfully via add_OnEvent")
            return
        except Exception as exc:
        ...
        try:
            ...
            self._trace_handler_attached = True
            logger.debug("Event handler attached successfully via OnEvent +=")
        except Exception as exc:
    ...
    def _handle_trace_event(self, args: Any) -> None:
        try:
            # EventClass is a TraceEventClass enum - convert to string
            event_class = getattr(args, "EventClass", None)
            if event_class is not None:
                event_name = str(event_class)
            else:
                event_name = ""

            session_id = str(getattr(args, "SessionID", "") or "")

            # Duration can be null/None - handle gracefully
            try:
                duration_ms = float(getattr(args, "Duration", 0) or 0)
            except (TypeError, ValueError):
                duration_ms = 0.0

            # CpuTime can also be null
            try:
            ...
            logger.debug("Trace event received: %s (session: %s, duration: %sms)",
                        event_name, session_id[:8] if session_id else "none", duration_ms)
        except Exception as exc:
    ...
    def _summarize_events(self, events: List[Dict[str, Any]], fallback_ms: float) -> Optional[Dict[str, Any]]:
        if not events:
            return None
        # NOTE: We don't filter by session_id here because:
        # 1. SessionTrace already filters to the current session
        # 2. We use index-based snapshots to isolate query-specific events
        # 3. The ADOMD SessionID and AMO SessionTrace session IDs may differ
        total_ms = None
        se_ms = 0.0
    ...
    def start_session_trace(self, query_executor: Optional[Any] = None) -> bool:
        ...
        try:
            ...
            # **IMPORTANT**: SessionTrace doesn't have an Events collection
            # It captures ALL events by default. We filter in _handle_trace_event instead.
            # The OnEvent handler attached earlier will receive all trace events.
            logger.debug("SessionTrace will capture all events (no event filtering at trace level)")
